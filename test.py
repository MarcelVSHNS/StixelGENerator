...
def main():
    ...
    if config['explore_data']:
        ...
        for i in range(5):
            plot_points_on_image(images=sample.images,
                                 laser_points=sample.laser_points,
                                 view=i)
    ...
    laser_stixel = []
    for i in range(5):
        laser_stixel.append(get_stixel_from_laser_data(laser_points=sample.laser_points))
        # Draw Stixel on image
        if config['explore_data']:
            plot_points_on_image(images=sample.images,
                                 laser_points=laser_stixel,
                                 view=0)
